...
(function ($) {
  ...
  $(document).ready(function () {
    ...
    $(".quantity-right-plus").on("click", function () {
      var $qty = $(this).closest(".qty-box").find('input[name="quantity"]');
      var currentVal = parseInt($qty.val());
      if (!isNaN(currentVal) && currentVal < 99) {
        // Giới hạn tối đa là 99
    ...
    $('input[name="quantity"]').on("input", function () {
      ...
      // Nếu số 0 ở đầu, xóa bỏ
      if (value.length > 1 && value.startsWith("0")) {
        value = value.replace(/^0+/, "");
      }

      // Gán lại giá trị tạm thời
      $(this).val(value);
    });

    // Kiểm tra giá trị khi mất focus
    $('input[name="quantity"]').on("blur", function () {
      var value = $(this).val();

      // Nếu trống hoặc không hợp lệ, đặt giá trị mặc định là 1
      if (value === "" || isNaN(parseInt(value)) || parseInt(value) < 1) {
        $(this).val("1");
      } else if (parseInt(value) > 99) {
        // Nếu lớn hơn 99, đặt lại thành 99
        $(this).val("99");
      }
    });
  });
